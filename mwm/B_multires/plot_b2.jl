# plot_b2.jl  --  maps for the b2 multi-resolution error CORRECTION test.
#
# Decoupled from the heavy integration: reads the NetCDF written by b2_probe.jl and
# produces PNG maps with CairoMakie (headless). Can be run standalone:
#
#     julia --project=. plot_b2.jl output_b2/b2_fields.nc output_b2
#
# or imported and called as plot_b2(ncfile, outdir).
...
# symmetric diverging map of one [lon,lat] variable, shared colour scale optional
...
function plot_b2(ncfile::AbstractString, outdir::AbstractString)
    mkpath(outdir)
    ...
    _map!(fig, (1, 5), lon, lat, Zco, "Z* T$tlo + ΔF", "m"; amax = amaxZ)
    p = joinpath(outdir, "b2_Zstar_three.png"); save(p, fig); push!(paths, p)
    ...
    _map!(fig2, (1, 3), lon, lat, rZc, "ΔZ* residual: T$tlo+ΔF - target", "m"; amax = amaxR)
    p = joinpath(outdir, "b2_Zstar_residual.png"); save(p, fig2); push!(paths, p)
    ...
    for (fld, unit) in (("T", "K"), ("U", "m/s"))
        ...
        _map!(fig3, (1, 3), lon, lat, rc, "Δ$(fld)* residual: +ΔF - target", unit; amax = amax)
        p = joinpath(outdir, "b2_$(fld)star_residual.png"); save(p, fig3); push!(paths, p)
    end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ncfile = length(ARGS) >= 1 ? ARGS[1] : error("usage: julia plot_b2.jl <b2_fields.nc> [outdir]")
    outdir = length(ARGS) >= 2 ? ARGS[2] : dirname(ncfile)
    paths = plot_b2(ncfile, outdir)
